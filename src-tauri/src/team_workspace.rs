...
/// 注入到主智能体 system prompt 末尾（`stream_pi_prompt` 内 `--append-system-prompt`）。
pub fn build_workspace_preface(app: &AppHandle, workspace_id: &str) -> Result<String, String> {
    let conn = crate::storage_conn(app)?;
    ...
    s.push_str("- **应用内委派（默认）**：通过下方「委派协议」与工具 `nineclaw_delegate_to_agent`，由 NineClaw 在本机用独立 Pi 子会话调用目标成员，会话线程与团队上下文一致。\n");
    s.push_str("- **虾 / HTTP 对等（跨实例）**：若成员在其它机器或另一 NineClaw 实例上，可使用 `POST {基址}/nineclaw/v1/inbound`，请求体 `protocol: nineclaw-peer`、`fromAgentId`、`toAgentId`、`threadId`、`text`；详见应用内文档「虾/外部互通」。\n\n");

    s.push_str("## 委派协议\n");
    s.push_str("将子任务交给成员智能体有两种协议：\n\n");
    ...
    s.push_str("- `nineclaw_delegate_to_agent` — 后台委派子智能体（应用侧通过 Pi 子会话执行）\n");
...
pub fn run_delegate_with_provider(
    ...
    run_delegate_with_provider_events(app, workspace_id, target_agent_id, task, provider, None)
}
...
pub fn run_delegate_with_provider_events(
    ...
    match outcome {
        PiProcessOutcome::Completed(r) => Ok(r.full_text),
        PiProcessOutcome::Aborted => Err("委派被中断".to_string()),
    }
}
